...
#define VAR(nr, min, max, bins, initbin) \
        if (threadIdx.y == (nr * WORK / NR_VARS)) {\
                localData[NR_CONSTS + NR_RECS + 2 * NR_VARS + nr] = localData[NR_CONSTS + NR_RECS + nr] = min  + ((real)initbin + extractNumber()) * (max - min) / (real)bins; \
...
#define REC(nr, cst) \
	if (threadIdx.y == (cst * WORK / NR_CONSTS)) {\
		localData[NR_CONSTS + nr] = 1 / localData[cst];\
	}
...
#define Y(n) localData[NR_CONSTS + n]
#define X(n) localData[NR_CONSTS + NR_RECS + 2 * NR_VARS + n]
#define AX(n) localData[NR_CONSTS + NR_RECS + 2 * NR_VARS + n]
#define XX xx_delta
#define DIV /
#define MUL *
#define PLUS +
...
#define AEQUATION(nr, formula, list) if (threadIdx.y  == (nr - NR_VARS) * WORK / NR_AVARS) localData[NR_CONSTS + NR_RECS + 2 * NR_VARS + nr] = (formula); 
...
volatile __device__ unsigned int syncs[128]; // assumes less than 64 SMs

#define SYNC_WORKERS(which) if ((threadIdx.x % 32) == 0) {atomicInc((unsigned int *)syncs + blockIdx.x + which * 64, WORK * ((PAR_SIMS + 31) / 32) - 1); while (syncs[blockIdx.x + which * 64] != 0);}


__device__ void RungeKutta(real * buffer_base) {
	real * localData = buffer_base;
...
#define K(n) localData[n]
#define Y(n) localData[NR_CONSTS + n]
#define X(n) localData[NR_CONSTS + NR_RECS + n]
#define XX xx_delta
#define DIV /
#define MUL *
#define PLUS +
...
#define EQUATION(nr, formula, list) \
	if (threadIdx.y  == (current_weight * WORK / total_weight)) {\
		RK(localData[NR_CONSTS + NR_RECS + NR_VARS + nr], X(nr), formula);\
	}\
...
__device__ void Increments(real * buffer_base) {
#define CONST(nr, rest...) 
#define REC(nr, cst)
#define VAR(nr, rest...)
#define AVAR(nr, rest...)
...
#define EQUATION(nr, formula, list) \
	if (threadIdx.y == (current_weight * WORK / total_weight)) {\
		buffer_base[NR_CONSTS + NR_RECS + nr] += buffer_base[NR_CONSTS + NR_RECS + NR_VARS + nr]; \
	}\
	current_weight = current_weight + weight_ ## nr;
...
#define VAR(nr, min, max, bins, initbin) if (threadIdx.y == (WORK + (nr * ACCESS / (NR_VARS + NR_AVARS)))) { \
	bin = 0; \
	real val = localData[NR_CONSTS + NR_RECS + 2 * NR_VARS + nr];\
...
#define BIN_THREADS (ACCESS * gridDim.x * PAR_SIMS)

#include "incBins.cu"

#define EQUATION(nr, formula, list) if (valid && (threadIdx.y == WORK + (nr * ACCESS / (NR_VARS + NR_AVARS)))) incBin(data_out, offsetof(struct xctr, x ## nr ## ctr [block] list [newx_bins[nr + (NR_VARS + NR_AVARS) * threadIdx.x]]), (blockIdx.x * PAR_SIMS + threadIdx.x) * ACCESS + threadIdx.y - WORK);

#define AEQUATION(nr, formula, list) if (valid && (threadIdx.y == WORK + (nr * ACCESS / (NR_VARS + NR_AVARS)))) incBin(data_out, offsetof(struct xctr, x ## nr ## ctr [block] list [newx_bins[nr + (NR_VARS + NR_AVARS) * threadIdx.x]]),  (blockIdx.x * PAR_SIMS + threadIdx.x) * ACCESS + threadIdx.y - WORK);
...
__global__ void __launch_bounds__(PAR_SIMS * (WORK + ACCESS), 1) kernel(void *data_out, int traces, int seed, int thread_step) {
	__shared__ unsigned int *k_bins, *x_bins, *newx_bins;
			...
			if (threadIdx.y < WORK) {
				if (block < BLOCKS - 1) {
					for (int t = 0; t < STEPS; t++) {
						RungeKutta(buffer_base);
						SYNC_WORKERS(0)
						Increments(buffer_base);
//						SYNC_WORKERS(1)
					}
				}
			} else {
			...
			if (block < BLOCKS - 1) for (int j = threadIdx.y; j < NR_VARS; j+= WORK + ACCESS) 
				buffer_base[NR_CONSTS + NR_RECS + 2 * NR_VARS + j] = buffer_base[NR_CONSTS + NR_RECS + j];
			__syncthreads();
		}
	}
}
...
void simulation(struct cudaDeviceProp *props, void *data_out, int traces, unsigned int seed) {
	...
	dim3 grid(props->multiProcessorCount);                                 // defines the number of blocks
        ...
        int thread_step = COALESCE((NR_CONSTS + NR_RECS + NR_VARS + NR_VARS + NR_VARS + NR_AVARS)); // coalescing is done at int level
	...
	cudaThreadSetLimit(cudaLimitMallocHeapSize, props->multiProcessorCount * 2 * PAR_SIMS * ((WORK + ACCESS) * 626 + NR_CONSTS + NR_VARS + NR_VARS + NR_AVARS) * sizeof(unsigned int));
