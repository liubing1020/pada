...
int main(int argc, char **argv) {
	...
	unsigned int *binoffset_gpu;
	unsigned int *binoffset_host;
	cudaEvent_t start, stop;
	bins_host = (struct xctr *)malloc(sizeof(struct xctr));
	struct cudaDeviceProp props;	
	cudaGetDeviceProperties(&props, WHICH_DEVICE);

	_step = PAR_SIMS * props.multiProcessorCount;
	_step = (TRACES - TRACES % _step) / _step + 1;
	_count = (NR_VARS + NR_AVARS - (NR_VARS + NR_AVARS) % ACCESS) / ACCESS +1;
	_count = _count * _step * BLOCKS;
	//_count is the largest count of the times incBin() called per thd
	_jump = PAR_SIMS * props.multiProcessorCount * ACCESS;

	if(!useMultiGPU || MSstatus==SLAVE){
		binoffset_host = (unsigned int *)malloc(sizeof(unsigned int*) * _count * _jump);
		cudaMalloc((void **)&binoffset_gpu, sizeof(unsigned int*) * (_count + 1) * _jump);
		cudaMemset(binoffset_gpu, 0, sizeof(unsigned int*) * (_count + 1) * _jump);
		...
		simulation(&props, binoffset_gpu, TRACES, WHICH_DEVICE);

		cudaEventRecord(stop, 0);
		cudaEventSynchronize(stop);
		printf("Status: %s\n", cudaGetErrorString(cudaGetLastError()));
		cudaMemcpy(binoffset_host, (unsigned int*)binoffset_gpu + _jump, sizeof(unsigned int*) * _count * _jump, cudaMemcpyDeviceToHost);
		cudaThreadSynchronize();
	...
	if(MSstatus==MASTER){
		...
		_step = PAR_SIMS * props.multiProcessorCount;
		_step = (masterthd._traces - masterthd._traces % _step) / _step + 1;
		_count = (NR_VARS + NR_AVARS - (NR_VARS + NR_AVARS) % ACCESS) / ACCESS +1;
		_count = _count * _step * BLOCKS;
	
		binoffset_host = (unsigned int *)malloc(sizeof(unsigned int*) * _count * _jump);
		masterthd._data_h=binoffset_host;
		...
		//collect output from slave gpus and upate bins
		FILE *binpartfile;
		char binfilename[255];
		unsigned int *binoffsetpart;
		binoffsetpart=(unsigned int *)malloc(sizeof(unsigned int*) * _count * _jump);
		...
		for(int i=1;i<useMultiGPU;i++){
			...
			if(binpartfile!=NULL){
				fread(binoffsetpart,sizeof(unsigned int*) * _count * _jump,1,binpartfile);
				fclose(binpartfile);
				unsigned int *u_bin_host = ( unsigned int *)bins_host;
				for(int j=0;j<_count * _jump;j++){
					if(binoffsetpart[j]==0) continue;
					*(u_bin_host+binoffsetpart[j]-1)+=1;
				}
			}
		}
	}
	...
	//update bins
	if(!useMultiGPU || MSstatus==MASTER){
		unsigned int *u_bin_host = ( unsigned int *)bins_host;
		unsigned int *binoffset_u=(unsigned int *)binoffset_host;
		for(int j=0;j<_count * _jump;j++){
			if(binoffset_u[j]==0) continue;
			*(u_bin_host+binoffset_u[j]-1)+=1;
		}
	}
...
#include "WriteFiles.c"
		...
		fwrite((unsigned int*)binoffset_host, sizeof(unsigned int*) * _count * _jump,1,tempfile);
		fclose(tempfile);
	}
	cudaFree(binoffset_gpu);
	free(bins_host);
