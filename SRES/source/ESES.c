...
void ESInitialIndividual(ESIndividual **indvdl, ESParameter *param)
{
  ...
  for(i=0; i<dim; i++)
  {
    (*indvdl)->op[i] = round(ShareRand(lb[i], ub[i]));  // [lb]: 121709
...
void ESPrintOp(ESIndividual *indvdl, ESParameter *param)
{
  ...
  if(trsfm == NULL)
    for(i=0; i<dim; i++)
      printf("\t%d", lround(indvdl->op[i])); // [lb]
  else
    for(i=0; i<dim; i++)
      if(trsfm[i] == NULL)
        printf("\t%d", lround(indvdl->op[i])); // [lb]
      else
        printf("\t%d", lround((trsfm[i])(indvdl->op[i]))); //[lb]
...
void ESMutate(ESPopulation * population, ESParameter *param)
{
  ...
  for(i=miu-1; i<lambda; i++)
  {
    ...
    for(j=0; j<dim; j++)
    {
      tmp = indvdl->sp[j] *exp(tau_ *randscalar +tau*randvec[j]); // update sp
      if( tmp > spb[j] )
        tmp = spb[j];
      indvdl->sp[j] = tmp;
    }
  ...
  for(i=0; i<miu-1; i++)
  {
    indvdl = population->member[i];
    for(j=0; j<dim; j++)
      indvdl->op[j] = round(indvdl->op[j] + gamma*(op_[0][j] - op_[i+1][j])); // [lb]:121709
  }
  for(i=miu-1; i<lambda; i++)
  {
    indvdl = population->member[i];
    for(j=0; j<dim; j++)
      indvdl->op[j] = round(indvdl->op[j] + indvdl->sp[j] * ShareNormalRand(0, 1)); // [lb]: 121709
  }
