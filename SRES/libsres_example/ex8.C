...
{
  double pi = 3.1415926515;

  (*f) = -pow(sin(2*pi*transform(x[0])),3)*sin(2*pi*transform(x[1]))/(pow(transform(x[0]),3)*(transform(x[0])+transform(x[1])));
  g[0] = transform(x[0])*transform(x[0])-transform(x[1])+1;
  g[1] = 1-transform(x[0])+(transform(x[1])-4)*(transform(x[1])-4);
