...
void fitness(double *x, double *f, double *g)
{
  ...
  (*f) = -(100.0-(transform(x[0])-5)*(transform(x[0])-5)-(transform(x[1])-5)*(transform(x[1])-5)-(transform(x[2])-5)*(transform(x[2])-5))/100.0;

  for(p=1;p<=9;p++)
    for(q=1;q<=9;q++)
      for(r=1;r<=9;r++)
      {
        csn = (transform(x[0])-p)*(transform(x[0])-p)+(transform(x[1])-q)*(transform(x[1])-q)+(transform(x[2])-r)*(transform(x[2])-r)-0.0625;
        if(csn <=0)
        {
          g[0] = csn;
          return;
