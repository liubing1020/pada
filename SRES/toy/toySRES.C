...
//[SPA]: include the FF file here, remember to remove its main
#include "toy_ff0.c"
 ...
 *********************************************************************/
  ...
  es = esDefESSlash;
  constraint = 1;
  dim = 3; //[SPA]: number of unknown parameters
  //[SPA]: miu, lambda, gen determine the how long you want to run 
  miu = 10;
  lambda = 30;
  gen = 10; 
  ub = NULL;
 ...
 *********************************************************************/
  ...
  for(i=0;i<dim;i++){
    //[SPA]: set up search space, e.g. each paramter have 5 bin
    lb[i] = 0; ub[i] = 4;
  }
...
void fitness(double *x, double *f, double *g)
{
  double value = 0.0;
  int i;
  int k[3]; // [SPA]: setup number of unknown parameters
  for(i=0;i<3;i++) // [SPA]: setup number of unknown parameters
    k[i] = (int)x[i];
  
  value=eval(k); // [SPA]: compute the objective value using FF
  //printf("%d %d %d = %f\n",k[0],k[1],k[2],value);
  //printf("%E\n",value);
